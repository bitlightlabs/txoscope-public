...
type TestOrchestrator = Orchestrator<InMemoryStateStore>;
...
fn initialize(store: &mut InMemoryStateStore) {
...
fn base_store() -> InMemoryStateStore {
	let mut store = InMemoryStateStore::default();
	initialize(&mut store);
	store
}

fn prepare(store: InMemoryStateStore) -> TestOrchestrator {
...
fn reserve_input(store: &mut InMemoryStateStore) {
